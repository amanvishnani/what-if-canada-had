...
export const Dashboard: React.FC<DashboardProps> = ({ accounts, transactions }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      {/* Welcome Banner */}
      <div className="bg-canDark rounded-none md:rounded-lg p-8 text-white shadow-lg relative overflow-hidden border-l-8 border-canRed">
      ...
      {/* Account Cards */}
      <div>
        <h3 className="text-xl font-bold text-canDark mb-4 flex items-center gap-2">
          Connected Institutions
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {accounts.map((account) => (
            <div key={account.id} className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-canRed transition-all">
              <div className="flex justify-between items-start mb-4">
                <BankLogo bankName={account.bankName} className="w-12 h-12" />
                <span className="text-xs text-gray-400 font-mono bg-gray-50 px-2 py-1 rounded">{account.accountNumber}</span>
              </div>
              <div className="mb-2">
                <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold">{account.accountType}</p>
                <h3 className="text-2xl font-bold text-gray-900 tracking-tight">
                  ${Math.abs(account.balance).toLocaleString('en-CA', { minimumFractionDigits: 2 })}
                </h3>
              </div>
              <div className="flex justify-between items-center pt-2 border-t border-gray-50">
                <p className="text-sm font-medium text-gray-600">{account.bankName}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            <h3 className="font-bold text-lg text-canDark">Unified Transaction Stream</h3>
            ...
            {transactions.map((t) => (
              <div key={t.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-canDark">
                    <DollarSign size={18} />
                  </div>
                  <div>
                    <p className="font-bold text-sm text-gray-800">{t.merchant}</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="font-bold text-lg text-canDark mb-6">Spending Analysis</h3>
          <div className="h-64">
          ...
          <div className="mt-4 space-y-3">
            {categoryData.slice(0, 3).map((entry: any, index: number) => (
              <div key={index} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0">
