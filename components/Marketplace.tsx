...
export const Marketplace: React.FC = () => {
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      <div className="bg-canDark text-white p-8 rounded-lg relative overflow-hidden border-l-8 border-yellow-400">
      ...
      <h3 className="text-xl font-bold text-canDark border-b border-gray-200 pb-2">Personalized Opportunities</h3>
      
      <div className="grid gap-4">
        {OFFERS.map((offer) => (
          <div key={offer.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-6 hover:border-canRed transition-all cursor-pointer group">
            
            <div className="flex items-center gap-6 flex-1">
              <BankLogo bankName={offer.provider} className="w-16 h-16 shrink-0" />
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-bold bg-green-100 text-green-800 px-2 py-0.5 rounded uppercase tracking-wide">
                    {offer.matchScore}% Match
                  </span>
                  <span className="text-xs text-gray-500 font-medium">{offer.type}</span>
                </div>
                <h4 className="font-bold text-xl text-canDark">{offer.product}</h4>
                <p className="text-gray-500 font-medium">{offer.provider}</p>
              </div>
            </div>

            <div className="flex-1 md:border-l md:border-r border-gray-100 md:px-6 flex flex-col justify-center">
            ...
            <div className="w-full md:w-auto">
              <button className="w-full md:w-auto flex items-center justify-center gap-2 bg-canDark text-white px-6 py-3 rounded font-bold group-hover:bg-canRed transition-colors shadow-sm">
      ...
      <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 mt-8 flex gap-4 items-start">
        <AlertCircle className="text-canDark shrink-0" size={24} />
        <div>
           <h4 className="font-bold text-canDark mb-1">Consumer Protection</h4>
